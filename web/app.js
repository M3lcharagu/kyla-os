...
async function runAgent(prompt){
  ...
  try{
    const r=await fetch(base+'/v1/run',{
      ...
      body:JSON.stringify({room:activeRoom,agent:activeAgent,prompt})
    });
    const j=await r.json().catch(()=>({ok:false,reply:'Bad JSON from bridge'}));
    if(pending) pending.remove();
    const reply=j.reply||j.error||'(empty reply)';
    addMsg(
      `KYLA / ${j.agent||activeAgent} · ${j.room||activeRoom}${j.ms!=null?` · ${j.ms}ms`:''}`,
      reply,
      j.ok===false?'error':''
    );
  }catch(e){
